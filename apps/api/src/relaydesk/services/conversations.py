...
def record(
    ...
    ``actor`` is ``None`` for activity generated by inbound mail rather than
    a signed-in user; ``actor_name`` lets that case still carry a readable
    name (falling back to "Relaydesk") without inventing a ``User`` row.
    """
    session.add(
        ActivityEvent(
            workspace_id=conversation.workspace_id,
            ...
            actor_name=actor_name or (actor.name if actor else "Relaydesk"),
            kind=kind,
            verb=verb,
            value=value,
            status=status,
